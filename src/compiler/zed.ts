...
export async function compileZedSkills(skills: SkillFile[], projectRoot: string): Promise<void> {
  const outputRoot = path.join(projectRoot, ".zed", "ai-rules");
  await fs.ensureDir(outputRoot);

  for (const skill of skills) {
    const fileName = `nymor-${skill.id}.md`;
    const outputPath = path.join(outputRoot, fileName);
    const content = renderZedRule(skill);
    await fs.writeFile(outputPath, content, "utf8");
  }
}

export function renderZedRule(skill: SkillFile): string {
