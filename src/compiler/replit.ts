...
export async function compileReplitSkills(skills: SkillFile[], projectRoot: string): Promise<void> {
  const outputRoot = path.join(projectRoot, ".replit", "ai-instructions");
  await fs.ensureDir(outputRoot);

  for (const skill of skills) {
    const fileName = `nymor-${skill.id}.md`;
    const outputPath = path.join(outputRoot, fileName);
    const content = renderReplitInstructions(skill);
    await fs.writeFile(outputPath, content, "utf8");
  }
}

export function renderReplitInstructions(skill: SkillFile): string {
