...
export async function compileCodySkills(skills: SkillFile[], projectRoot: string): Promise<void> {
  const outputRoot = path.join(projectRoot, ".cody", "instructions");
  await fs.ensureDir(outputRoot);

  for (const skill of skills) {
    const fileName = `nymor-${skill.id}.md`;
    const outputPath = path.join(outputRoot, fileName);
    const content = renderCodyInstructions(skill);
    await fs.writeFile(outputPath, content, "utf8");
  }
}

export function renderCodyInstructions(skill: SkillFile): string {
