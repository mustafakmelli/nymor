...
export async function compileTabnineSkills(skills: SkillFile[], projectRoot: string): Promise<void> {
  const outputRoot = path.join(projectRoot, ".tabnine", "instructions");
  await fs.ensureDir(outputRoot);

  for (const skill of skills) {
    const fileName = `nymor-${skill.id}.md`;
    const outputPath = path.join(outputRoot, fileName);
    const content = renderTabnineInstructions(skill);
    await fs.writeFile(outputPath, content, "utf8");
  }
}

export function renderTabnineInstructions(skill: SkillFile): string {
