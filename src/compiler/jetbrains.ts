...
export async function compileJetBrainsSkills(skills: SkillFile[], projectRoot: string): Promise<void> {
  const outputRoot = path.join(projectRoot, ".idea", "ai-assistant");
  await fs.ensureDir(outputRoot);

  for (const skill of skills) {
    const fileName = `nymor-${skill.id}.md`;
    const outputPath = path.join(outputRoot, fileName);
    const content = renderJetBrainsInstructions(skill);
    await fs.writeFile(outputPath, content, "utf8");
  }
}

export function renderJetBrainsInstructions(skill: SkillFile): string {
